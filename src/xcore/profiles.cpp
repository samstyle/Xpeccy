...
//	prf_load_cmos(nprof,  profileStatePath(nprof, ".cmos"));  // TODO: delayed
//	prf_load_nvram(nprof, profileStatePath(nprof, ".nvram")); // TODO: delayed
...
int prfSave(std::string nm) {
	...
	writeKV(out, "A",    getDiskString(comp->dif->flp[0]));
	writeKV(out, "B",    getDiskString(comp->dif->flp[1]));
	writeKV(out, "C",    getDiskString(comp->dif->flp[2]));
	writeKV(out, "D",    getDiskString(comp->dif->flp[3]));
