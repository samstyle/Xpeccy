...
//	cpu->f = 0;
	...
	cpu->flgZ = !cpu->ltw;
	cpu->flgA = !!iop_add_h[((val & 8) >> 1) | ((add & 8) >> 2) | ((cpu->ltw & 8) >> 3)];
...
unsigned char iop_sub(CPU* cpu, unsigned char val, unsigned char sub) {
	...
	cpu->flgS = !!(cpu->ltw & 0x80);
	cpu->flgZ = !cpu->ltw;
	cpu->flgA = !!iop_sub_h[((val & 8) >> 1) | ((sub & 8) >> 2) | ((cpu->ltw & 8) >> 3)];
	cpu->flgP = parity(cpu->ltw);
	cpu->flgC = !!cpu->htw;
	return cpu->ltw;
}
...
void iop_76(CPU* cpu) {cpu->regPC--; if (!cpu->flgHALT) {cpu->flgHALT = 1; cpu_irq(cpu, IRQ_CPU_HALT);}}
...
// c0: rnz
void iop_c0(CPU* cpu) {if (!cpu->flgZ) cpu->regPC = iop_pop(cpu);}
// c1: pop b
...
// c8: rz
void iop_c8(CPU* cpu) {if (cpu->flgZ) cpu->regPC = iop_pop(cpu);}
// c9: ret
...
// cf: rst 1
// d0: rnc
void iop_d0(CPU* cpu) {if (!cpu->flgC) cpu->regPC = iop_pop(cpu);}
// d1: pop d
...
// d7: rst 2
// d8: rc
void iop_d8(CPU* cpu) {if (cpu->flgC) cpu->regPC = iop_pop(cpu);}
// d9: ret*
// da: jc nn
void iop_da(CPU* cpu) {
	cpu->t += 3;
...
// df: rst 3
// e0: rpo
void iop_e0(CPU* cpu) {if (!cpu->flgP) cpu->regPC = iop_pop(cpu);}
// e1: pop h
...
// e7: rst 4
// e8: rpe
void iop_e8(CPU* cpu) {if (cpu->flgP) cpu->regPC = iop_pop(cpu);}
// e9: pchl
...
// ef: rst 5
// f0: rp
void iop_f0(CPU* cpu) {if (!cpu->flgS) cpu->regPC = iop_pop(cpu);}
// f1: pop psw
void iop_f1(CPU* cpu) {
	cpu->tmpw = iop_pop(cpu);
	cpu->regA = cpu->htw;
...
// f7: rst 6
// f8: rm
void iop_f8(CPU* cpu) {if (cpu->flgS) cpu->regPC = iop_pop(cpu);}
// f9: sphl
void iop_f9(CPU* cpu) {
	cpu->regSP = cpu->regHL;
}
