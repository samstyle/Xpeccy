...
int slt_adr_dum(xCartridge* slt, int mt, int adr) {
	return 0;
}

int slt_rd_dum(xCartridge* slt, int mt, int adr, int radr) {
	return 0xff;
}

void slt_wr_dum(xCartridge* slt, int mt, int adr, int radr, int val) {
}

// MSX mappers

int slt_msx_all_rd(xCartridge* slot, int mt, int adr, int radr) {
	if (!slot->data) return 0xff;
	return slot->data[radr & slot->memMask];
}

// no mapper
int slt_msx_nomap_adr(xCartridge* slot, int mt, int adr) {
...
// konami 4
int slt_msx_kon4_adr(xCartridge* slot, int mt, int adr) {
...
void slt_msx_kon4_wr(xCartridge* slot, int mt, int adr, int radr, int val) {
	switch(adr) {
...
// konami 5
int slt_msx_kon5_adr(xCartridge* slot, int mt, int adr) {
	int bnk = ((adr & 0x2000) >> 13) | ((adr & 0x8000) >> 14);
	bnk = slot->memMap[bnk];
	int radr = (bnk << 13) | (adr & 0x1fff);
	radr &= slot->memMask;
...
void slt_msx_kon5_wr(xCartridge* slot, int mt, int adr, int radr, int val) {
	switch (adr & 0xf800) {
...
// ascii8
// rd = konami5 rd
void slt_msx_asc8_wr(xCartridge* slot, int mt, int adr, int radr, int val) {
	switch (adr & 0xf800) {
...
// ascii16
int slt_msx_asc16_adr(xCartridge* slot, int mt, int adr) {
...
void slt_msx_asc16_wr(xCartridge* slot, int mt, int adr, int radr, int val) {
	switch (adr & 0xf800) {
...
int slt_gb_all_adr(xCartridge* slot, int mt, int adr) {
	int radr;
	if (adr & 0x8000) {		// ram
...
int slt_gb_all_rd(xCartridge* slot, int mt, int adr, int radr) {
...
void slt_gb_mbc1_wr(xCartridge* slot, int mt, int adr, int radr, int val) {
	switch (adr & 0xe000) {
...
void slt_gb_mbc2_wr(xCartridge* slot, int mt, int adr, int radr, int val) {
	switch (adr & 0xe000) {
...
void slt_gb_mbc3_wr(xCartridge* slot, int mt, int adr, int radr, int val) {
	switch (adr & 0xe000) {
		case 0x0000:		// 0000..1fff : xA = ram enable
			slot->ramen = ((val & 0x0f) == 0x0a) ? 1 : 0;
			break;
...
void slt_gb_mbc5_wr(xCartridge* slot, int mt, int adr, int radr, int val) {
	switch(adr & 0xf000) {
...
int slt_nes_all_rd(xCartridge* slot, int mt, int adr, int radr) {
	unsigned char res = 0xff;
	switch (mt) {
		case SLT_PRG:
			if (slot->data)
...
int nes_nt_vadr(xCartridge* slot, int adr) {
	if (adr & 0x2000) {
		switch (slot->mirror) {
			case NES_NT_SINGLE:
...
int slt_nes_nrom_adr(xCartridge* slot, int mt, int adr) {
	int radr = 0;
	switch (mt) {
		case SLT_PRG:
			radr = adr & 0x7fff;
...
int slt_nes_mmc1_adr(xCartridge* slot, int mt, int adr) {
	int radr = -1;
	switch (mt) {
		case SLT_PRG:
...
void slt_nes_mmc1_wr(xCartridge* slot, int mt, int adr, int radr, int val) {
	switch (mt) {
		case SLT_PRG:
			if (val & 0x80) {
...
int slt_nes_unrom_adr(xCartridge* slot, int mt, int adr) {
	int radr = -1;
	switch (mt) {
...
void slt_nes_unrom_wr(xCartridge* slot, int mt, int adr, int radr, int val) {
...
int slt_nes_cnrom_adr(xCartridge* slot, int mt, int adr) {
	int radr = -1;
	switch (mt) {
		case SLT_PRG:
			radr = adr & 0x7fff;
...
void slt_nes_cnrom_wr(xCartridge* slot, int mt, int adr, int radr, int val) {
	if (mt != SLT_PRG) return;
...
int slt_nes_mmc3_adr(xCartridge* slot, int mt, int adr) {
	int radr = -1;
	switch (mt) {
		case SLT_PRG:
...
void slt_nes_mmc3_wr(xCartridge* slot, int mt, int adr, int radr, int val) {
	switch (mt) {
		case SLT_PRG:
			switch(adr & 0xe001) {
				case 0x8000:
...
void slt_nes_mmc3_chk(xCartridge* slot, int adr) {
...
int slt_nes_aorom_adr(xCartridge* slot, int mt, int adr) {
	int radr = -1;
	switch(mt) {
		case SLT_PRG:
...
void slt_nes_aorom_wr(xCartridge* slot, int mt, int adr, int radr, int val) {
	if (mt != SLT_PRG) return;
	slot->reg00 = val & 7;
...
void slt_nes_camerica_wr(xCartridge* slot, int mt, int adr, int radr, int val) {
	if (mt == SLT_PRG) {
...
int sltRead(xCartridge* slt, int mt, int adr) {
	int res = -1;
...
void sltWrite(xCartridge* slt, int mt, int adr, int val) {
	if (!slt->core) return;
...
void sltChecker(xCartridge* slt, int adr) {
	if (!slt->data) return;
	if (!slt->core) return;
