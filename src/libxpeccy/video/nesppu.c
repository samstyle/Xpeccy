...
void ppuDraw(nesPPU* ppu) {
	col = 0;
	if (ppu->ray->x < 8) {
		pap = ppu->bgleft8 ? 0 : ppu->bgline[(ppu->ray->x + ppu->scx) & 0x1ff];		// bg color
		ink = ppu->spleft8 ? 0 : ppu->spline[ppu->ray->x];				// spr color
	} else {
		pap = ppu->bgline[(ppu->ray->x + ppu->scx) & 0x1ff];
		ink = ppu->spline[ppu->ray->x];
	}
	if (ppu->prline[ppu->ray->x]) {			// spr behind bg
		col = (pap & 3) ? pap : ink;
	} else {					// spr above bg
		col = (ink & 3) ? ink : pap;
	}
		...
		while ((cnt < 8) && (lin < 64)) {
			...
			if ((x < 0xfa) && (y < 0xf0)) {		// sprite is visible
				...
				if (sln < shi) {		// sprite is crossing current line
					if (lin == 0) ppu->sp0hit = 1;		// sprite 0 hit
					if (flag & 0x80) sln = shi - sln;		// VFlip
					if (ppu->bigspr) {
