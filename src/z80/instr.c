unsigned char xRLC(Spec* p,unsigned short ad) {
...
unsigned char xRR(Spec* p,unsigned short ad) {
	p->cpu->mptr = ad + (signed char)p->cpu->dlt;
...
unsigned char xSLA(Spec* p,unsigned short ad) {
	p->cpu->mptr = ad + (signed char)p->cpu->dlt;
...
unsigned char xSRA(Spec* p,unsigned short ad) {
	p->cpu->mptr = ad + (signed char)p->cpu->dlt;
...
unsigned char xSLI(Spec* p,unsigned short ad) {
	p->cpu->mptr = ad + (signed char)p->cpu->dlt;
...
unsigned char xSRL(Spec* p,unsigned short ad) {
	p->cpu->mptr = ad + (signed char)p->cpu->dlt;
...
unsigned char xRES(Spec* p, unsigned short ad, unsigned char msk) {
	p->cpu->mptr = ad + (signed char)p->cpu->dlt;
...
unsigned char xSET(Spec* p, unsigned short ad, unsigned char msk) {
	p->cpu->mptr = ad + (signed char)p->cpu->dlt;
	unsigned char res = p->mem->rd(p->cpu->mptr) | msk;
	p->mem->wr(p->cpu->mptr,res);
	return res;
...
void rpSBC(Spec* p, unsigned short ar) {
...
void rpADC(Spec* p, unsigned short ar) {
	int dp = ((p->cpu->f & FC)?1:0);
	p->cpu->mptr = p->cpu->hl + 1;
	p->cpu->adr = p->cpu->hl + ar + dp;
	int urs = p->cpu->hl + ar + dp;
